...
PLAYING,
GAME_OVER,
INSTRUCTIONS  // Thêm state INSTRUCTIONS
};
...
SDL_Texture* bg1Layer1 = window.loadTexture("Nen.png");
SDL_Texture* bg1Layer2 = window.loadTexture("mayxa.png");
SDL_Texture* bg1Layer3 = window.loadTexture("moutain.png");
SDL_Texture* bg1Layer4 = window.loadTexture("may1.png");
SDL_Texture* bg1Layer5 = window.loadTexture("may2.png");

SDL_Texture* bg2Layer1 = window.loadTexture("Back21.png");
SDL_Texture* bg2Layer2 = window.loadTexture("Back22.png");
SDL_Texture* bg2Layer3 = window.loadTexture("Back23.png");
SDL_Texture* bg2Layer4 = window.loadTexture("Back24.png");
SDL_Texture* bg2Layer5 = window.loadTexture("Back25.png");

SDL_Texture* bg3Layer1 = window.loadTexture("Back31.png");
SDL_Texture* bg3Layer2 = window.loadTexture("Back32.png");
SDL_Texture* bg3Layer3 = window.loadTexture("Back33.png");
SDL_Texture* bg3Layer4 = window.loadTexture("Back34.png");
SDL_Texture* bg3Layer5 = window.loadTexture("Back35.png");
...
birdTextures.push_back(window.loadTexture("frame-1.png"));
birdTextures.push_back(window.loadTexture("frame-2.png"));
...
// Load pipe textures
SDL_Texture* topPipeTexture = window.loadTexture("pipetren.png");
SDL_Texture* bottomPipeTexture = window.loadTexture("pipeduoi.png");
...
PipeManager pipeManager(topPipeTexture, bottomPipeTexture);
  Vacham vacham;
// Create score and boards
SDL_Color scoreColor = { 0, 51, 51 };
Score score(window.gRender, "arial.ttf", 35, scoreColor);

SDL_Color board1Color = { 255, 100, 0 };
SDL_Color board2Color = { 0, 0, 51 };
SDL_Color board3Color = { 0, 0, 51 };
SDL_Color board4Color = { 0, 0, 51 };
Board board1(window.gRender, "game.ttf", 135, board1Color);
Board board2(window.gRender, "text.ttf", 30, board2Color);
Board board3(window.gRender, "text.ttf", 30, board3Color);
Board board4(window.gRender, "text.ttf", 30, board4Color);
Board high(window.gRender, "arial.ttf", 35, scoreColor);

// Load audio
Mix_Music* backgroundMusic = Mix_LoadMUS("background.mp3");
Mix_Chunk* flapSound = Mix_LoadWAV("flap.wav");
...
SDL_Texture* BangSelect = window.loadTexture("bangselect.png");
SDL_Texture* Banghuongdan = window.loadTexture("Bangguide.png");
SDL_Rect Rectstart;
...
while (GameRunning) {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        switch (currentState) {
                ...
                if (event.type == SDL_KEYDOWN) {
                    if (event.key.keysym.sym == SDLK_1) {
                        ...
                        currentState = PLAYING;
                         ...
                         currentState = INSTRUCTIONS; // chuyển sang hướng dẫn
                    }
                }
                break;
            ...
            case GAME_OVER:
                ...
                break;
                 case INSTRUCTIONS: // Thêm case cho INSTRUCTIONS
                if (event.type == SDL_KEYDOWN) {
    ...
    switch (currentState) {
        ...
        case PLAYING: {
            ...
            // Update scroll based on selected map
            ...
            switch (selectedMap) {
                ...
                case 3:
                    ...
                    break;
                default:
                    // Handle the case where no map is selected
                    break;
            }
            ...
            if (!gameOver && vacham.CheckVaCham(bird, pipeManager, score))
            {
                ...
                finalScore = score.getScore(); // Lưu điểm số cuối cùng
            ...
            case INSTRUCTIONS:
                if (event.type == SDL_KEYDOWN) {
                    if (event.key.keysym.sym == SDLK_ESCAPE || event.key.keysym.sym == SDLK_e)
                        {
                        currentState = MENU; // Quay lại MENU khi nhấn ESC hoặc E
                    }
                }
                window.clear();
                window.copy(bg1Layer1, Rectstart);
